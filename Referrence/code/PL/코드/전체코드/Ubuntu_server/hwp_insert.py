            ...
            if len(stream)>1:
                ...
                if 'PS' in stream[1].upper():
                    ...
                    if len(xor_key_re.findall(str(post_script_decompress_data)))>0:
                        ...
                        os.remove(os.path.basename(self.FilePath)+stream[1])
...
class hwp_insert:
    ...
    def insert_file_ssdeep(self):
        file_ssdeep_hash=ssdeep.hash_from_file(self.file_path)

        chunksize, chunk, double_chunk = file_ssdeep_hash.split(':')
        ...
        document = {
              ...
              "ssdeep" : file_ssdeep_hash,
              "sha256" : self.sha256,
        }
        self.q_master.insert(document=document, index="hwp-file-ssdeep", doc_type="record")
...
def main_inserts():
    ...
    for groups in dirname:
        for group_samples in os.listdir(os.path.join(sample_path,groups)):
            ...
            insert_object.insert_file_ssdeep()
