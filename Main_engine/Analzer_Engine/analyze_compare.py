...
class AnalyzeFlowchart:
    ...
    def compare_bb_constants(self, stand_list, target_list, s_hash_dict,
                             t_hash_dict):  # 엔진에선 인자 2개만 필요, 뒤에 두개는 self.idbAll["constant"] 식으로..

        ##################################################################################################################
        # dict는 프로젝트 엔진에서는 미 사용해야함. self ~
        ##################################################################################################################
        s_fname, s_sAddr = stand_list
        t_fname, t_sAddr = target_list
        ##################################################################################################################
        ############################ n_hash_dict 함수는 메인엔진 self 변수에 맞게 변경되어야 함. ############################
        ##################################################################################################################
